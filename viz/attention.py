...
def show(inp_text, enc_atts):
    # enc_att = resize(enc_atts)
    attention = _get_attention(inp_text, enc_atts)
...
def _get_attention(inp_text, enc_atts):
  ...
  Args:
    ...
    enc_atts:
        [{
            'index': ,
            'values': [num_heads, enc_length, enc_length]
        }]
  ...
  """
    ...
    def get_inp_inp_attention(layer):
        att = np.transpose(enc_atts[layer]['values'][:, :max_length, :max_length], (0, 2, 1))
